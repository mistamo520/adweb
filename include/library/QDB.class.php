...
class QDB
{
	...
	static public function &Instance()
	{
		...
		{ 
			...
			{
				$config = self::$config;
				self::$mInstance = new $class($config[0],$config[1],$config[2],$config[3]);
			}
			else
			{
	...
	{
		require(dirname(dirname(__FILE__)).'/configure/qdb.php');
		if(empty($host)||empty($user)||empty($pass)||empty($name))
		{
			if (isset($qdb) && $qdb) {
				$host = (string) $qdb['host'];
				$user = (string) $qdb['user'];
				$pass = (string) $qdb['pass'];
				$name = (string) $qdb['name'];
			} else {
		...
		{
			self::$config = array($host,$user,$pass,$name);
		}
	...
	static public function Query( $sql , $flag = false)
	{
		self::Instance();
		if ( self::$mDebug )
		{
		...
		//$flag标志是否需要事物处理
		if( $flag ){
			@mysql_query( "SET AUTOCOMMIT=0" , self::$mConnection) ;
			@mysql_query( "BEGIN" , self::$mConnection) ;
			$sql = trim($sql , ';') ;
			$sqlArr = explode(';',$sql );
			$sign = true ;
			$realnum = 0 ;
			foreach ( $sqlArr as $val ){
				$ret = @mysql_query( $val , self::$mConnection ) ;
				$realnum += @mysql_query( "select row_count()") ;
				if( !$ret ){
					$sign = false ;
					break;
				}
...
//			$result = @mysql_query( $sql , self::$$mConnection ) ;
			if( $sign ){
				@mysql_query( 'COMMIT' , self::$mConnection);
				self::Close();
				return true;
			}else {
				@mysql_query('ROLLBACK' , self::$mConnection) ;
			}
		}else{
			$result = @mysql_query( $sql, self::$mConnection );
			self::$mCount++;
	...
	static public function UpdateQuery( $sql , $flag = false)
	{
		self::Instance();
		if ( self::$mDebug )
		{
	...
	static public function transQuery() {
